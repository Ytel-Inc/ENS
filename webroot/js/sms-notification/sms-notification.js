...
angular.module('ensApp.smsNotification', ['ngRoute'])
    ...
    .controller('SmsNotificationCtrl', ['$scope', '$http', '$firebaseObject', '$timeout', '$filter', function($scope, $http, $firebaseObject, $timeout, $filter) {
            var scope = $scope;
            ...
            scope.sendSms = function() {
                ...
                $http.post('/Sms/ajaxSendSmsMessage.json', {sms: scope.sms}).then(function(response) {
                    ...
                    if (response.data.response.status) {
                        ...
                        currentQueueObj.$watch(function() {
                            scope.queueArray = $filter('toArray')(currentQueueObj.numbers);
                            
                            if( scope.queueArray ) {
                                var inQueue = $filter('filter')(scope.queueArray, {status: null}),
                                    sending = $filter('filter')(scope.queueArray, {status: 2}),
                                    sent = $filter('filter')(scope.queueArray, {status: 3}),
                                    fail = $filter('filter')(scope.queueArray, {status: 4});

                                var total = scope.queueArray ? scope.queueArray.length : 0;
                                var count = [
                                    sending ? sending.length : 0,
                                    sent ? sent.length : 0,
                                    fail ? fail.length : 0
                                ];
                                var inQueue = total - count[0] - count[1] - count[2];
